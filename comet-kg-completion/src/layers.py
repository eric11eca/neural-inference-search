        ...
        if self.self_loop:
            if isinstance(self, MultiHeadGATLayer):
                self.loop_weight = nn.Parameter(
                    torch.Tensor(in_feat, out_feat * 8))
            else:
                self.loop_weight = nn.Parameter(
                    torch.Tensor(in_feat, out_feat))
...
class WGCNAttentionLayer(RGCNLayer):
    ...
    def attn_reduce(self, nodes):
        attn_vector = torch.bmm(
            nodes.mailbox['msg'], nodes.data['h'].unsqueeze(2))
        attn_probs = torch.softmax(attn_vector, dim=1)
        attn_weighted_msgs = nodes.mailbox['msg'] * \
            attn_probs.expand_as(nodes.mailbox['msg'])
