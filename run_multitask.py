...
def evaluate(task_split, model, predict_dir):
    ...
    for dataset in datasets:
        nli_test_data = py_io.read_jsonl(f"data/{dataset}/nli_val.jsonl")   
        nli_output = t5_saint_generator.generate(nli_test_data)

        evid_test_data = py_io.read_jsonl(f"data/{dataset}/evid_val.jsonl")  
        evid_output = t5_saint_generator.generate(evid_test_data)

        py_io.write_jsonl(nli_output, os.path.join(predict_dir, "nli_pred.jsonl"))
        py_io.write_jsonl(evid_output, os.path.join(predict_dir, "evid_pred.jsonl"))
...
if __name__=='__main__':
    ...
    parser.add_argument("--task_split", default="snli")
    ...
    parser.add_argument("--max_epochs", default=5, type=float,
