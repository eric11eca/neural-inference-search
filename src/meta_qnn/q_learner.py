    ...
    def save_to_csv(self, q_csv_path):
        ...
        for start_state_list in self.q.keys():
            ...
            for to_state_ix in range(len(self.q[start_state_list]['actions'])):
                ...
                utility.append(self.q[start_state_list]
                               ['utilities'][to_state_ix])
...
class QLearner:
    def __init__(self,
        ...
        self.state = se.State('start', 0, 1, 0, 0,
                              args.patch_size, 0, 0) if not state else state
        self.qstore = QValues()
    ...
    def generate_net(self, epsilon=None, dataset=None):
        ...
        if net_string in self.replay_dictionary['net'].values:
            ...
            hard_best_val = self.replay_dictionary[self.replay_dictionary['net']
                                                   == net_string]['reward'].values[0]
            train_flag = self.replay_dictionary[self.replay_dictionary['net']
                                                == net_string]['train_flag'].values[0]

            self.replay_dictionary = self.replay_dictionary.append(
                pd.DataFrame(
                    [[net_string, spp_size, hard_best_val, self.epsilon, train_flag]],
                    columns=['net', 'spp_size', 'reward', 'epsilon', 'train_flag']),
                ignore_index=True)
            self.count += 1
            ...
            spp_size, hard_best_val, train_flag = self.__train_val_net(
                state_list, self.state_space_parameters, dataset)
    ...
    def __train_val_net(self, state_list, state_space_parameters, dataset):
        ...
        model = net(state_list, state_space_parameters, num_classes,
                    net_input, self.args.batch_norm, self.args.drop_out_drop)
        ...
        print('available:{}'.format((self.gpu_mem_0.total_mem -
                                     self.gpu_mem_0.total_mem*self.gpu_mem_0.get_mem_util())/1024.))
        ...
        while epoch < self.args.epochs:
            train(dataset, model, criterion, epoch, optimizer,
                  lr_scheduler, self.device, self.args)
            prec = validate(dataset, model, criterion,
                            epoch, self.device, self.args)
    ...
    def sample_replay_for_update(self):
        ...
        state_list = self.stringutils.convert_model_string_to_states(
            cnn_parse('net', net))
        # if train_flag:
        self.__update_q_value_sequence(
            state_list, self.__accuracy_to_reward(reward_best_val/100.))

        for i in range(self.state_space_parameters.replay_number-1):
            net = np.random.choice(self.replay_dictionary['net'])
            reward_best_val = self.replay_dictionary[self.replay_dictionary['net']
                                                     == net]['reward'].values[0]
            train_flag = self.replay_dictionary[self.replay_dictionary['net']
                                                == net]['train_flag'].values[0]
            state_list = self.stringutils.convert_model_string_to_states(
                cnn_parse('net', net))
            # if train_flag == True:
            self.__update_q_value_sequence(
    ...
    def __update_q_value(self, start_state, to_state, reward):
        ...
        max_over_next_states = max(self.qstore.q[to_state.as_tuple(
