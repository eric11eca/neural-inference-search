...
class GreedySampler(Sampler):
    ...
    def append_batch(self, X, next_idx, mask):
        ...
        next_mask = torch.cat(
            [mask, torch.ones(X.size(0), 1, device=mask.device)], 1)
...
class TopKSampler(Sampler):
    ...
    def generate_sequence(self, batch, model, data_loader, start_idx, end_len):
        ...
        next_pos = XMB[:, -1:, 1] + 1
        next_x = torch.cat(
            (indices.view(self.opt.eval.k, -1), next_pos), -1).unsqueeze(1)
        XMB = torch.cat((XMB, next_x), 1)
        MMB = torch.cat(
            [MMB, torch.ones(XMB.size(0), 1, device=MMB.device)], 1)
        ...
        for _ in range(end_len):
            ...
            next_idx = next_idx * (1 - ended).long() + \
                ended.long() * self.end_token
...
class BeamSampler(TopKSampler):
    def __init__(self, opt, data_loader, batch_mode=True, scorer=None):
        ...
        self.kill_mask = torch.ones(
            opt.eval.bs, opt.eval.bs).to(cfg.device) * 9000
        self.kill_mask[:, 0] = 0
    ...
    def make_batch(self, X):
        ...
        pos_enc = np.arange(n_vocab + n_special, n_vocab +
                            n_special + X.shape[-1])
    ...
    def generate_sequence(self, batch, model, data_loader, start_idx, end_len):
        ...
        for _ in range(end_len):
            ...
            hypothesis_mask = expanded_ended * \
                self.kill_mask + (1 - expanded_ended)
            ...
            beam_lls, top_beam_idxs = (
                hyp_beam_lls / temp_counts).topk(self.opt.eval.bs)
